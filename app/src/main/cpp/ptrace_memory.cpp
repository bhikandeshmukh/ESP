...
#define LOGI(...)                                                              \
  __android_log_print(ANDROID_LOG_INFO, "PtraceMem", __VA_ARGS__)
#define LOGE(...)                                                              \
  __android_log_print(ANDROID_LOG_ERROR, "PtraceMem", __VA_ARGS__)

// Global instance (extern in header)
PtraceMemory *g_ptraceMem = nullptr;

PtraceMemory::PtraceMemory() : m_pid(-1), m_attached(false) {}

PtraceMemory::~PtraceMemory() { detach(); }
...
bool PtraceMemory::writeMemory(uintptr_t address, const void *buffer,
                               size_t size) {
  if (m_pid <= 0 || !buffer || size == 0) {
    return false;
  }
