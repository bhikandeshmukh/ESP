...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "RemoteJNI", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "RemoteJNI", __VA_ARGS__)

// Global PtraceMemory instance for RemoteService
static PtraceMemory* g_ptraceMem = nullptr;

extern "C" {

JNIEXPORT jboolean JNICALL
Java_com_gameesp_RemoteService_nativeAttach(JNIEnv* env, jobject thiz, jint pid) {
...
Java_com_gameesp_RemoteService_nativeReadBytes(JNIEnv* env, jobject thiz, jlong address, jint size) {
...
JNIEXPORT jboolean JNICALL
Java_com_gameesp_RemoteService_nativeWriteBytes(JNIEnv* env, jobject thiz, jlong address, jbyteArray data) {
    if (!g_ptraceMem || !data) {
        return JNI_FALSE;
    }
